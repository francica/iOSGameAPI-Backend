...
from BoardManagement.board_segment import BoardSegment
from BoardManagement.game_instance import GameInstance
import json
from BoardManagement.pen import Pen
...
class BoardManagement:
    ...
    def add_new_game_board(self, pen_color,pen_width,room_id):
        ...
        self.game_board[room_id]=GameInstance()

        new_pen=Pen(pen_color,pen_width)

        new_board=BoardSegment(new_pen,self.game_board[room_id].board_id,room_id)
        new_board.initialize_new_board()


        self.game_board[room_id].board_segment_list.append(new_board)
        self.game_board[room_id].board_id+=1
    ...
    def update_pen(self,json_obj):
        ...
        new_board=BoardSegment(new_pen,self.game_board[json_str['room_id']].board_id,json_str['room_id'])

        self.game_board[json_str['room_id']].board_segment_list.append(new_board)
        self.game_board[json_str['room_id']].board_id+=1
    ...
    def update_board(self,json_obj):
        ...
        json_str=json.loads(json_obj)
        self.game_board[json_str['room_id']].board_segment_list[len(self.game_board[json_str['room_id']].board_segment_list)-1].update_board_pixel(json_str['start_point'],json_str['end_point'])
    ...
    def find_boards(self,room_id,board_id):
        ...
        if board_id==self.game_board[room_id].board_segment_list[len(self.game_board[room_id].board_segment_list)-1].board_id:
        ...
        for board_index in range(0, len(self.game_board[room_id].board_segment_list)):

            # get all the boards from the board_id to the latest board
            if self.game_board[room_id].board_segment_list[board_index].board_id==board_id:
                board_diff=self.game_board[room_id].board_segment_list[board_index+1:]
        ...
        if len(board_diff)!=0:
            ...
            for board in board_diff:
                ...
                start_point_list.append(board.pixels_changed[0])
                end_point_list.append(board.pixels_changed[1])
    ...
    def remove_game(self,room_id):
        ...
        self.game_board.pop(room_id,None)
